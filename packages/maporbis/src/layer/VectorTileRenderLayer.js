...
// PointsMaterial,
BufferGeometry, Float32BufferAttribute, LineSegments, LineBasicMaterial, InstancedMesh, DynamicDrawUsage, Vector2, CircleGeometry, MeshBasicMaterial, PlaneGeometry, TextureLoader, DoubleSide, Mesh } from 'three';
...
export class VectorTileRenderLayer extends OverlayLayer {
    ...
    processTileData(tile, data) {
        ...
        const prjCenter = map.prjcenter;
        // Phase B: Use Layout for coordinate transformation
        // 阶段 B：使用 Layout 进行坐标转换
        const layoutFeatures = this._layout.layoutTileFeatures(vectorData, map, prjCenter, globalPaintRules, (filter, properties, layerName, geometryType) => this._evaluateFilter(filter, properties, layerName, geometryType));
     ...
     * 使用 Worker 处理瓦片数据（异步）
     */
    async processTileDataAsync(tile, data) {
        const map = this.getMap();
        const tileKey = `${tile.z}-${tile.x}-${tile.y}`;
        // Check cache
        const existingFeatures = this._tileFeatureMap.get(tileKey);
        ...
        try {
            ...
            const layoutFeatures = this._layout.layoutTileFeatures(vectorData, map, prjCenter, this.paint, (filter, properties, layerName, geometryType) => this._evaluateFilter(filter, properties, layerName, geometryType));
            // Step 2: Send pre-transformed features to Worker for bucket grouping + vertex generation
            // 第二步：将已转换坐标的要素发送给 Worker 进行分桶 + 顶点生成
            const result = await this._workerManager.processTile(tileKey, layoutFeatures, this.paint);
            ...
            result.points.forEach(pointData => {
                if (pointData.instances.length === 0)
                    return;
        ...
        if (useWideLine) {
            ...
            if (!material) {
                material = new LineMaterial({
                    ...
                    opacity: opacity,
                    dashed: Array.isArray(config.dashArray) && config.dashArray.length > 0,
                    dashScale: 1,
                ...
                meshMaterial.onBeforeCompile = (shader) => {
                    shader.vertexShader = shader.vertexShader.replace('#include <project_vertex>', `
                        vec4 mvPosition = vec4( transformed, 1.0 );
                            ...
                            bool isSizeAttenuation = ${config.sizeAttenuation !== false};
                        ...
                        `);
                    if (config.sizeAttenuation === false) {
     ...
     * 从顶点和索引创建面网格
     */
    _createFillMesh(vertices, indices, config) {
        const geometry = new BufferGeometry();
        geometry.setAttribute('position', new Float32BufferAttribute(vertices, 3));
        geometry.setIndex(indices);
        const material = new MeshBasicMaterial({
            color: config.fillColor || config.color || 0xffffff,
            transparent: true,
            opacity: config.fillOpacity ?? config.opacity ?? 1,
            side: DoubleSide,
            depthWrite: false
        });
     ...
     * @param tileKey Tile identifier. 瓦片标识符。
     */
    removeFeaturesByTileKey(tileKey) {
        this._removeFeaturesByTileKey(tileKey);
    }
    _removeFeaturesByTileKey(tileKey) {
        const features = this._tileFeatureMap.get(tileKey);
     ...
     */
    _createFeatureInstance(geometry, type, paint, properties) {
    ...
    // --- 生命周期、样式和更新 ---
    _getAnchorShaderInjection(config) {
        ...
        if (config.anchor) {
            if (typeof config.anchor === 'string') {
                switch (config.anchor) {
                    ...
                    case 'bottom-left':
                        anchor = [0.0, 0.0];
                        break; // 修正：Mapbox/Sprite 习惯 bottom-left 可能是 (0,0) ? 
    ...
    setLineColor(color) {
        VectorTileRenderLayer._materialCache.forEach((mat) => {
...
class MergedFeature extends Feature {
     ...
     *                    此要素使用的顶点数量
     */
    constructor(sharedMesh, options, _startIndex, _vertexCount) {
...
class InstancedFeature extends MergedFeature {
    ...
    constructor(sharedMesh, options, instanceId, count = 1) {
