...
export class Sky extends Mesh {
  declare isSky: boolean;
  private _material: SkyMaterial;
  ...
  update(params: SkyParams) {
    ...
    // 天空参数 - 直接赋值
    if (params.turbidity !== undefined) u.turbidity.value = params.turbidity;
    if (params.rayleigh !== undefined) u.rayleigh.value = params.rayleigh;
    ...
    if (params.mult !== undefined) u.mult.value = params.mult;
    // THICKNESS: 参数值 / 3e4（three-tile-doc 的转换）
    if (params.THICKNESS !== undefined)
      u.THICKNESS.value = params.THICKNESS / 3e4;
    if (params.ABSORPTION !== undefined) u.ABSORPTION.value = params.ABSORPTION;
    ...
    if (params.weaken !== undefined) u.weaken.value = params.weaken;
    if (params.coverage !== undefined) u.coverage.value = params.coverage;
    // speed: 参数值 / 10（three-tile-doc 的转换）
    if (params.speed !== undefined) u.speed.value = params.speed / 10.0;
    if (params.wind !== undefined)
      u.wind.value = new Vector3(params.wind.x, params.wind.y, params.wind.z);
    if (params.skylineF !== undefined) u.skylineF.value = params.skylineF;
    // curve: 1 - 参数值（three-tile-doc 的转换）
    if (params.curve !== undefined) u.curve.value = 1 - params.curve;

    // 雾参数：用于地平线附近过渡
    if (params.fogColor !== undefined) {
      if (
        typeof params.fogColor === "number" ||
    ...
    if (params.fogDensity !== undefined) u.fogDensity.value = params.fogDensity;

    // 星星参数
    if (params.starIntensity !== undefined) u.uStarIntensity.value = params.starIntensity;

    // 月亮参数
    if (params.moonIntensity !== undefined) u.uMoonIntensity.value = params.moonIntensity;
    if (params.moonAngularRadius !== undefined) u.uMoonAngularRadius.value = params.moonAngularRadius;
