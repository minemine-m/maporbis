...
import { BaseTileLayer } from "./TileLayer/TileLayer";
...
export class VectorTileLayer extends BaseTileLayer {
    ...
    _feaList = [];
    _collision = false;
    _renderAltitude = 0; // Rendering altitude 渲染高度
    ...
    constructor(id, options) {
        ...
        this._rootTile.setDataOnlyMode(true);
    ...
    */
    _setupDataModeAndListenersForChildren() {
        ...
        // 监听新瓦片创建事件
        this._rootTile.addEventListener('tile-created', (event) => {
    ...
    _addShownListenerToTile(tile) {
        const listener = (event) => {
            const shownTile = event.tile;
    ...
    _addUnloadListenerToTile(tile) {
        const listener = (event) => {
            ...
            const t = event.tile || event.target;
     ...
     */
    //@ts-ignore
    _addHiddenListenerToTile(tile) {
        const listener = (event) => {
            const hiddenTile = event.tile;
    ...
    */
    _setupLifeCycleListeners() {
        ...
        // 此事件是在 rootTile 上派发的
        this._rootTile.addEventListener('tile-loaded', (event) => {
     ...
     * 从瓦片几何体中提取矢量数据
     */
    getVectorDataFromTile(tile) {
        if (!tile.geometry || !tile.getVectorData()) {
            return null;
        }
    ...
    getVisibleVectorTiles() {
        const result = [];
        this._rootTile.traverse(tile => {
            if (tile.isTile && tile.loaded && tile.inFrustum) {
                const tileKey = `${tile.z}-${tile.x}-${tile.y}`;
     ...
     * @private
     */
    _refreshVisibleTiles() {
        if (!this._renderer)
            return;
        this._rootTile.traverse((tile) => {
            if (tile.isTile && tile.loaded && tile.showing) {
                const tileKey = `${tile.z}-${tile.x}-${tile.y}`;
