 ...
 */
function evaluateFilter(filter, properties, layerName, geometryType) {
    ...
    if (op === 'all')
        return filter.slice(1).every((f) => evaluateFilter(f, properties, layerName, geometryType));
    if (op === 'any')
        return filter.slice(1).some((f) => evaluateFilter(f, properties, layerName, geometryType));
    if (op === 'none')
        return !filter.slice(1).some((f) => evaluateFilter(f, properties, layerName, geometryType));
    if (op === '!')
        return !evaluateFilter(filter[1], properties, layerName, geometryType);
    if (op === 'has')
 ...
 */
function processLineFeatures(features, paintRules) {
 ...
 */
function processPointFeatures(features, paintRules) {
    const bucketMap = new Map();
    for (const feature of features) {
 ...
 */
function processFillFeatures(features, paintRules) {
    const bucketMap = new Map();
    for (const feature of features) {
