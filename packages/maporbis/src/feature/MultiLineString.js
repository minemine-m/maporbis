...
export class MultiLineString extends Line {
    ...
    async _buildRenderObject() {
        ...
        if (this._paint) {
            ...
            // 为每条线创建单独的几何体
            for (const linePositions of _worldCoordinates) {
                ...
                lineObject.position.add(map?.prjcenter);
     ...
     * @private
     */
    async _createLineObject(paint, vertexPoints) {
        switch (paint.config.type) {
            ...
            case 'flow-texture':
                return await _createFlowTextureLine(paint.config, vertexPoints);
            default:
