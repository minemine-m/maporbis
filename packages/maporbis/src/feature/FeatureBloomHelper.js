...
export class FeatureBloomHelper {
     ...
     */
    setBloomConfig(enabled, options) {
     ...
     */
    applyStyleBloom(styleBloom) {
        if (styleBloom === undefined)
            return;
     ...
     */
    applyBloomToObject(root) {
        if (!this._bloomConfig)
            return;
     ...
     * 将发光效果应用到点材质
     */
    _applyBloomToPoints(child, enabled, intensity) {
     ...
     * 将发光效果应用到精灵材质
     */
    _applyBloomToSprite(child, enabled, intensity, color) {
        const mat = child.material;
        child.userData = child.userData || {};
        if (!child.userData.__bloomBackup) {
            child.userData.__bloomBackup = {
     ...
     * 将发光效果应用到Line2材质
     */
    _applyBloomToLine2(child, enabled, intensity, color) {
        const mat = child.material;
        child.userData = child.userData || {};
        if (!child.userData.__bloomBackup) {
            child.userData.__bloomBackup = {
     ...
     * 将发光效果应用到网格材质
     */
    _applyBloomToMesh(child, enabled, intensity, color) {
