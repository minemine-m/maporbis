import { CanvasTexture, FileLoader } from "three";
...
import { VectorFeatureTypes } from "../materials/vectorTileRenderer/IVectorTileRender";
/**
...
export class VectorTileTextureLoader extends AbstractMaterialLoader {
    ...
    dataType = "mvt";
    _loader = new FileLoader(TileLoaderFactory.manager);
    _render = new VectorTileRender();
    constructor() {
        super();
    ...
    async performLoad(url, context) {
        ...
        const data = (await this._loader.loadAsync(url));
        // Parse vector tile
        const vectorTile = new VectorTile(new Pbf(data));
        // Render to canvas
        const paint = context.source.paint;
    ...
    _renderLayer(ctx, layer, paint, scale = 1) {
        ctx.save();
    ...
    _renderFeature(ctx, feature, paint = {}, scale = 1) {
        const type = [
     ...
     */
    convertVectorTileToGeoJSON(vectorTile) {
        const features = [];
        for (const layerName in vectorTile.layers) {
            const layer = vectorTile.layers[layerName];
            for (let i = 0; i < layer.length; i++) {
    ...
    _convertToGeoJSONFeature(renderFeature, type) {
    ...
    _convertGeometryToGeoJSON(geometry, type) {
        switch (type) {
    ...
    _convertPointGeometry(geometry) {
        const coordinates = [];
    ...
    _convertPolygonGeometry(geometry) {
        const coordinates = [];
        let currentPolygon = [];
        for (const ring of geometry) {
            const polygonRing = [];
