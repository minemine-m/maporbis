...
export class Viewer extends ViewerBase {
    ...
    private stats: Stats;
    /** 动画回调集合 */
    private _animationCallbacks: Set<(delta: number, elapsedtime: number, context: Viewer) => void> = new Set();
    /** 雾效因子 */
    ...
    private async _loadHDRWithPMREM(scene: Scene, skyboxConfig: ViewerOptions['skybox']) {
        try {
            if (skyboxConfig) {
                ...
                hdrTexture.mapping = 303;  // 303 = EquirectangularReflectionMapping
    ...
    private _createControls(minDistance?: number, maxDistance?: number) {
        ...
        controls.addEventListener("change", () => {
            ...
            if (controls.maxDistance > maxFar * 0.95) controls.maxDistance = maxFar * 0.95;
     ...
     */
    private _createAuxLightInstance(x: number, y: number, z: number, intensity: number): DirectionalLight {
     ...
     * @returns 移除回调的函数
     */
    public addAnimationCallback(callback: (delta: number, elapsedtime: number, context: Viewer) => void): () => void {
