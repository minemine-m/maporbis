...
function getCategory(item) {
    ...
    if (item.tags) {
        const tag = item.tags.find(function(t) { return t.title === 'category'; });
...
function buildNav(members) {
    ...
    const categoriesConf = env.conf.templates.categories;
    
    if (categoriesConf) {
        const categoryNames = categoriesConf.split(',').map(function(s) { return s.trim(); });
        const categories = {};
        const uncategorized = [];
        ...
        const allItems = []
            .concat(members.modules)
            .concat(members.externals)
            .concat(members.namespaces)
            .concat(members.classes)
            .concat(members.interfaces)
            .concat(members.events)
            .concat(members.mixins);
            ...
            if (items && items.length) {
                ...
                items.forEach(function(item) {
                    if ( !hasOwnProp.call(seen, item.longname) ) {
                         ...
                         nav += '<li>' + linkto(item.longname, displayName.replace(/\b(module|event):/g, '')) + '</li>';
...
function buildClassNav(doclet, data) {
    let nav = '';
    nav += `<h5><a href="#main" style="color:#000">class : ${doclet.name}</a></h5>`;

    // Members
    const members = helper.find(data, {kind: 'member', memberof: doclet.longname});
    if (members && members.length) {
        nav += '<h5>Members</h5><ul>';
        members.forEach(m => {
            nav += `<li><a href="#${m.name}">${m.name}</a></li>`;
        });
        nav += '</ul>';
    }

    // Methods
    const methods = helper.find(data, {kind: 'function', memberof: doclet.longname});
    if (methods && methods.length) {
        nav += '<h5>Methods</h5><ul>';
        methods.forEach(m => {
            nav += `<li><a href="#${m.name}">${m.name}</a></li>`;
        });
        nav += '</ul>';
    ...
    // Events
    const events = helper.find(data, {kind: 'event', memberof: doclet.longname});
    if (events && events.length) {
        nav += '<h5>Events</h5><ul>';
        events.forEach(m => {
            const dispName = m.name.replace(/^event:/, '');
            nav += `<li><a href="#${m.name}">${dispName}</a></li>`;
        });
        nav += '</ul>';
    }
...
exports.publish = (taffyData, opts, tutorials) => {
    ...
    const globalNav = view.nav;
    view.navClass = 'page-nav'; // Default to page-nav
    ...
    Object.keys(helper.longnameToUrl).forEach(longname => {
        ...
        if (myClasses.length) {
            view.classNav = buildClassNav(myClasses[0], data);
            view.navClass = 'classnav';
            generate(`Class: ${myClasses[0].name}`, myClasses, helper.longnameToUrl[longname]);
            delete view.classNav;
            view.navClass = 'page-nav';
        }
